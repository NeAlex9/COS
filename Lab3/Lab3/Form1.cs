    ...
    public partial class Form1 : Form
    {
        ...
        private void Calculate(int freq, int min, int max)
        {
            ...
            if (Redraw == false || instSignal == null) { 
                ...
                {                                                 //360
                    hs = new HarmonicSignal(20, freq, Math.PI / 4, N, min, max, currentFiltrationType);
                }
                else
                {
                    double[] A = new double[8] { 1, 3, 4, 10, 11, 12, 14, 17 };
                    double[] ph = new double[7] { Math.PI / 6, Math.PI / 4, Math.PI / 3, Math.PI / 2, Math.PI, 3 * Math.PI / 4, Math.PI };
            ...
            for (int i = 0; i <= N - 1; i++)
            {
                targetCharts[0].Series[0].Points.AddXY(2 * Math.PI * i *10 / N, instSignal.signVal[i]);
                Summa[i] = instSignal.signVal[i];
            }

            for (int i = 0; i <= instSignal.resotorePoints - 1; i++)
            {
                targetCharts[0].Series[1].Points.AddXY(2 * Math.PI * i * 10 / instSignal.resotorePoints, instSignal.restoredSignal[i]);
                targetCharts[0].Series[2].Points.AddXY(2 * Math.PI * i * 10 / instSignal.resotorePoints, instSignal.restorednonphasedSignal[i]);
            }
